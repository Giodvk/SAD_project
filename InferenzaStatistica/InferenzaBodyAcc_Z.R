...
iqr_value <- IQR(body_acc_z, na.rm = TRUE)
n <- nrow(df)
bin_width <- 2 * iqr_value / (n^(1/3))

# Creazione dei bin
breaks <- seq(min(body_acc_z, na.rm = TRUE), 
              max(body_acc_z, na.rm = TRUE), 
              by = bin_width)

# Assegnazione delle osservazioni ai bin
cuts <- cut(body_acc_z, breaks = breaks, include.lowest = TRUE)
table(cuts)
...
mu <- mean(body_acc_z, na.rm = TRUE)
sigma <- sd(body_acc_z, na.rm = TRUE)
...
#La variabile secondo il test-chi quadrato può essere approssimata da una normale
#Procediamo a stimare gli intervalli di confidenza dei parametri
#In questo caso dobbiamo stimare gli intervalli di confidenza per la media e deviazione standard

#Dobbiamo utlizzare la varianza del campione per stimare mu, oltre ad una variabile di Student
# Alpha = 0.05
# 1 - alpha = 0.95

c1 <- mean(body_acc_z) - qt(1-alpha/2,df = length(body_acc_z) - 1) * sigma / length(body_acc_z)
c2 <- mean(body_acc_z) + qt(1-alpha/2,df = length(body_acc_z) - 1) * sigma / length(body_acc_z)
...
c1 <- length(body_acc_z) * var(body_acc_z)/qchisq(1 - alpha/2, df = length(body_acc_z) - 1)
c2 <- length(body_acc_z) * var(body_acc_z)/qchisq(alpha/2, df = length(body_acc_z) - 1)
