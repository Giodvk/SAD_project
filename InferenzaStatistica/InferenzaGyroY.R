...
iqr_value <- IQR(body_gyro_y, na.rm = TRUE)
n <- nrow(df)
bin_width <- 2 * iqr_value / (n^(1/3))

# Creazione dei bin
breaks <- seq(min(body_gyro_y, na.rm = TRUE), 
              max(body_gyro_y, na.rm = TRUE), 
              by = bin_width)
...
mu <- mean(body_acc_x, na.rm = TRUE)
sigma <- sd(body_acc_x, na.rm = TRUE)
